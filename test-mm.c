...
static int N = 0;  // should be a power of two 
...
void eval_perf(unsigned int s, unsigned int E, unsigned int b, char *r)
{
    ...
    for (i=0; i<func_counter; i++) {
        ...
        sprintf(cmd, "valgrind --tool=lackey --trace-mem=yes --log-fd=1 -v ./tracegen-mm -N %d -F %d  > trace.tmp", N,i);
...
void usage(char *argv[]){
    ...
    printf("  -N <cols>   Number of  matrix columns (max %d). power of 2\n", MAXN);
